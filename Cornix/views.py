...
def return_trade_history(user):
    ...
    my_portfolio = MyClient.objects.filter(
        myclient=MyUser).exclude(has_funds=False)

    if my_portfolio.exists():
        for client in my_portfolio:
            my_trades = MyOrder.objects.filter(
                client=client).order_by('-date_placed')[:9]
...
def usdt_to_btc(amount):
    btc = float(coin_gecko.get_price(ids='bitcoin',
                vs_currencies='usd')['bitcoin']['usd'])
...
def set_my_client_name(message):
    ...
    bot.send_message(
        chat_id=user_detail["chat_id"], text="Select the currency you want to trade:", reply_markup=payment_method())
...
def set_invest_amount(message):
    ...
    try:
        ...
        bot.send_message(
            chat_id=user_detail["chat_id"], text=detail, reply_markup=confirm_payment())
        ...
        msg = bot.send_message(
            user_detail["chat_id"], "Please enter a number", reply_markup=Cancel_btn())
        # REPEAT STEP
...
def process_transaction_hash(message):
    ...
    if message.text == "❌ Cancel":
        ...
        bot.send_message(user_detail["chat_id"], "Action terminated use the /start command to begin",
                         reply_markup=ReplyKeyboardRemove(selective=False))  # Main_Menu())
    else:
        msg = "Hello your transaction is being processed. Please wait"
        ...
        try:
            ...
            # if result == True:
            myclient = MyClient(myclient=MyUser, balance=MY_STATE["deposit_amount"], exchange=my_exchange,
                                client_name=MY_STATE["client_name"], investment_currency=payment_method, has_funds=False, start_trading=False)
            myclient.save()

            bot.send_message(user_detail["chat_id"], msg, reply_markup=ReplyKeyboardRemove(
                selective=False))  # Main_Menu())
            bot.send_message(
                user_detail["chat_id"], "Once your deposit is confirmed the bot will start trading", reply_markup=Main_Menu())
            STEP.objects.filter(
                user=user_detail["user_id"]).update(next_step="")

            notify_admin_payment(myclient.client_id,
...
def process_withdrawal_amount(message):
    ...
    if message.text == "❌ Cancel":
        ...
        try:
            ...
            my_clients = MyClient.objects.filter(
                myclient=MyUser).filter(balance__gte=withdrawal_amount)
            ...
            if my_clients.exists():
                ...
                bot.send_message(
                    chat_id=user_detail["chat_id"], text=msg, reply_markup=Cancel_btn())
                STEP.objects.filter(user=MyUser).update(
                    state=MY_STATE, next_step="GET_WITHDRAWAL_ADDRESS")

            else:
                msg = "Sorry but you don't have upto that amount yet"
                bot.send_message(
                    chat_id=user_detail["chat_id"], text=msg, reply_markup=Main_Menu())
...
def process_withdrawal_address(message):
    ...
    if message.text == "❌ Cancel":
		...
		"""
        bot.send_message(
            chat_id=user_detail["chat_id"], text=msg, reply_markup=confirm_withdrawal())
        STEP.objects.filter(user=user_detail["user_id"]).update(next_step="")
...
def notify_admin_payment(client_id, coin, txn_hash):
    ...
    for admin in admins:
        msg = f"@{client.myclient.user_name} tradeed  {client.balance} {coin} \n\n TXN HASH: <code>{txn_hash}</code>"
        try:
            client_id = hex(int(client.client_id.time_low))[2:]
            bot.send_message(admin.chat_id, msg, reply_markup=admin_approval(
                client.myclient, client_id))
        except Exception as e:
...
def notify_admin_withdrawal(client_id, withdrawal_amount, withdrawal_currency, withdrawal_address):
    ...
    for admin in admins:
        ...
        try:
            client_id = hex(int(client.client_id.time_low))[2:]
            bot.send_message(admin.chat_id, msg, reply_markup=admin_approve_withdrawal(
                client_id, withdrawal_amount))
        except Exception as e:
    ...
    try:
        ...
        if call.data.endswith("@client"):
            ...
            if q == "Binance" or q == "ByBit" or q == "Coinex":
                bot.edit_message_text(chat_id=chat_id, text=step_1_2_of_3.format(
                    q), message_id=call.message.message_id, reply_markup=new_client_step_1_2_of_3(q, show_menu))
            else:
                bot.edit_message_text(chat_id=chat_id, text=step_2_of_3, message_id=call.message.message_id,
                                      reply_markup=new_client_step_2_of_3(q, show_menu))

        elif call.data.startswith("option_"):
            MY_STATE["previuos_step"] = "1_2"
            q = call.data.split(":")[1]
            option = call.data.split(":")[0].split("option_")[1]
            MY_STATE["option"] = option.replace(
                "_", " ").replace(q.lower(), "").title()
            STEP.objects.filter(user=user).update(
                state=MY_STATE, next_step="SET_NAME")
            ...
            bot.edit_message_text(chat_id=chat_id, text="Select the currency you want to trade:",
                                  message_id=call.message.message_id, reply_markup=payment_method())
            ...
            if q == "home":
                ...
                bot.edit_message_text(
                    chat_id=chat_id, text=msg, message_id=call.message.message_id, reply_markup=home_key())

            elif q == "about":
                bot.edit_message_text(chat_id=chat_id, text=about_cornix_auto,
                                      message_id=call.message.message_id, reply_markup=about_cornix(step))
                ...
                bot.edit_message_text(chat_id=chat_id, text=my_portfolio.format(
                    btc_balance, usdt_balance), message_id=call.message.message_id, reply_markup=my_portfolios_btn())

            elif q == "1":
                bot.edit_message_text(chat_id=chat_id, text=step_1_of_3,
                                      message_id=call.message.message_id, reply_markup=new_client_step_1_of_3(show_menu))

            elif q == "2":
                bot.edit_message_text(chat_id=chat_id, text=step_2_of_3, message_id=call.message.message_id,
                                      reply_markup=new_client_step_2_of_3(exchange, show_menu))

            elif q == "1_2":
                bot.edit_message_text(chat_id=chat_id, text=step_1_2_of_3.format(
                    MY_STATE["exchange"]), message_id=call.message.message_id, reply_markup=new_client_step_1_2_of_3(exchange, show_menu))

            elif q == "bot_configs":
                bot.edit_message_text(chat_id=chat_id, text=bot_config,
                                      message_id=call.message.message_id, reply_markup=bot_configs())
            else:
                pass
        ...
        # THE BILLING PROGRAM STARTS HERE
            ...
            detail = f"Please Enter amount of {q} you want to trade"
            bot.send_message(chat_id=chat_id, text=detail,
                             reply_markup=Cancel_btn())
            STEP.objects.filter(user=user).update(
                next_step="GET_INVEST_AMOUNT")
        ...
        # BOT CONFIGS
            ...
            bot.edit_message_text(chat_id=chat_id, text=client_config,
                                  message_id=call.message.message_id, reply_markup=bot_configs_clients(my_clients))

        elif call.data.startswith("my_client:"):
            q = call.data.split(":")[1]
            # This is for a users client method
            my_client = MyClient.objects.filter(
                Q(client_id__icontains=q)).first()
            client_id = hex(int(my_client.client_id.time_low))[2:]
            bot.edit_message_text(chat_id=chat_id, text=configure_a_client,
                                  message_id=call.message.message_id, reply_markup=configs_clients(client_id))

        elif call.data.startswith("delete_client:"):
            q = call.data.split(":")[1]
            my_client = MyClient.objects.filter(Q(client_id__icontains=q)).first(
            ).delete()  # This is for a users client method
            bot.edit_message_text(chat_id=chat_id, text="Client has been deleted",
                                  message_id=call.message.message_id, reply_markup=back_to_configs())
        ...
        # MAIN MENU CALLBACK ACTION
            ...
            bot.edit_message_text(
                chat_id=chat_id, text=f"{trade_history}{my_trades}", message_id=call.message.message_id, reply_markup=Main_Menu())

        elif call.data == "my_portfolio_faq":
            bot.edit_message_text(chat_id=chat_id, text=portfolio_info,
                                  message_id=call.message.message_id, reply_markup=my_portfolios_info_btn())
            ...
            MyClient.objects.filter(myclient=subscriber).update(
                has_funds=True, start_trading=True)
            bot.send_message(
                subscriber.chat_id, "Congratulations You deposit is successful the bot will start trading now.")

            # This is for a users client method
            my_client = MyClient.objects.filter(
                Q(client_id__icontains=client_id)).first()

            msg = f"{call.message.text}\n\n Payment Approved ✅✅✅"
            bot.edit_message_text(chat_id=chat_id, text=msg, message_id=call.message.message_id,
                                  reply_markup=admin_approval(my_client.myclient, client_id))
			...
			"""
            MyClient.objects.filter(myclient=subscriber).update(
                has_funds=False, start_trading=False)
        ...
        # ABOUT CORNIX CALLBACK DATA
            ...
            bot.edit_message_text(chat_id=chat_id, text=about_cornix_auto,
                                  message_id=call.message.message_id, reply_markup=about_cornix())

        elif call.data == "about_cornix_E":
            bot.edit_message_text(chat_id=chat_id, text=about_cornix_E,
                                  message_id=call.message.message_id, reply_markup=about_cornix_back())

        elif call.data == "cornix_can_do":
            bot.edit_message_text(chat_id=chat_id, text=cornix_can_do,
                                  message_id=call.message.message_id, reply_markup=about_cornix_back())

        elif call.data == "made_for":
            bot.edit_message_text(chat_id=chat_id, text=made_for,
                                  message_id=call.message.message_id, reply_markup=about_cornix_back())

        elif call.data == "trade_for":
            bot.edit_message_text(chat_id=chat_id, text=trade_for,
                                  message_id=call.message.message_id, reply_markup=about_cornix_back())

        elif call.data == "cornix_features":
            bot.edit_message_text(chat_id=chat_id, text=cornix_features,
                                  message_id=call.message.message_id, reply_markup=about_cornix_back())
        ...
        # WITHDRAWAL FUNTION BEGINS
            ...
            bot.edit_message_text(
                chat_id=chat_id, text=msg, message_id=call.message.message_id, reply_markup=withdrawal_currency_opt())
            ...
            STEP.objects.filter(user=user).update(
                state=MY_STATE, next_step="GET_WITHDRAWAL_AMOUNT")
            ...
            withdrawal_amount = MY_STATE["WITHDRAWAL"]["withdrawal_amount"]
            my_clients = MyClient.objects.filter(
                myclient=user).filter(balance__gte=withdrawal_amount)
            if my_clients.exists():
                ...
                bot.send_message(
                    chat_id=chat_id, text="Withdrawal in progress please wait...")
                notify_admin_withdrawal(
                    client.client_id, withdrawal_amount, withdrawal_currency, withdrawal_address)
            else:
                bot.send_message(
                    chat_id=chat_id, text="Sorry not enough balance", reply_markup=Main_Menu())

        elif call.data == "cancel_withdrawal":
            bot.send_message(
                chat_id=chat_id, text="Withdrawal has been canceled", reply_markup=Main_Menu())
            ...
            bot.edit_message_text(chat_id=chat_id, text=msg, message_id=call.message.message_id,
                                  reply_markup=admin_approve_withdrawal(client_id, withdrawal_amount))
...
def start(message):
    ...
    if user_detail["msg_type"] == "private":
        try:
            ...
            user = User(
                user_name=user_detail["username"], chat_id=user_detail["chat_id"], user_id=user_detail["user_id"])
            user.save()
            notify_admin(user)
        ...
        bot.send_message(user_detail["chat_id"], step_1_of_3,
                         reply_markup=new_client_step_1_of_3(show_menu))
    else:
        pass
